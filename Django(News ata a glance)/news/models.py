...
class searchNews(object):
	...
	def search(self,search_list): #定義多重條件查詢方法
		...
		# for i in xrange(0,len(search_list)):
			# line += '{"content":{"$regex":\"'+search_list[i]+"\",'$options':'$i'}}," #會從tfidf的欄位去抓
		...
		#存取搜尋過後的全部新聞資料--------------------------------------------------------------
		for post in collection.find(eval(querry),{'_id':0}).sort("date",1).limit(3000):
		...
		if len(newsList) >300:        #當新聞數量超過300，把日期進行月份的整合，並且把title取消掉
			for post in collection2.find(eval(querry),{'_id':0}).limit(3000).sort("date",1):
